//
//  MineStudentInviteViewController.m
//  Bican
//
...
@interface MineStudentInviteViewController ()<UITableViewDelegate, UITableViewDataSource, DZNEmptyDataSetDelegate, DZNEmptyDataSetSource>
...
@implementation MineStudentInviteViewController
...
- (void)createButton
{
    ...
    self.groupButton.adjustsImageWhenHighlighted = NO;
    [self.groupButton setImage:[UIImage imageNamed:@"group"] forState:UIControlStateNormal];
    self.groupButton.hidden = YES;
    [self.groupButton addTarget:self action:@selector(gruopButtonAction) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:self.groupButton];

    [self.groupButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(self.view).offset(-GTH(88));
        make.width.height.mas_equalTo(GTH(88));
...
#pragma mark - 跳转多选页面
- (void)gruopButtonAction
{
    if (self.dataSourceArray.count == 0) {
        return;
    }
    InviteChooseViewController *inviteChooseVC = [[InviteChooseViewController alloc] init];
    [self.inviteVC pushVC:inviteChooseVC animated:YES IsNeedLogin:YES];
}
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section
{
    ...
    self.acceptButton.frame = CGRectMake(ScreenWidth - GTW(30) - button_width, GTH(15), button_width, GTH(50));
