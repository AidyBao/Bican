...
@interface CorrectViewController ()<UIScrollViewDelegate>

@property (nonatomic, strong) MineCorrectViewController *mineCorrectVC;
@property (nonatomic, strong) NowCorrectViewController *nowCorrectVC;
...
{
    if (!_titleArray) {
        _titleArray = [NSMutableArray arrayWithObjects:@"正在评阅", @"我评阅的", nil];
    }
    return _titleArray;
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    //通知刷新
    [[NSNotificationCenter defaultCenter] postNotificationName:@"childReload" object:nil];
...
{
    //正在评阅
    self.nowCorrectVC = [[NowCorrectViewController alloc] init];
    self.nowCorrectVC.correctVC = self;
    self.nowCorrectVC.view.frame = CGRectMake(0, 0, self.bigScrollView.frame.size.width, self.bigScrollView.frame.size.height);
    [self.bigScrollView addSubview:self.nowCorrectVC.view];
    
    //我评阅的
    self.mineCorrectVC = [[MineCorrectViewController alloc] init];
    self.mineCorrectVC.correctVC = self;
    self.mineCorrectVC.view.frame = CGRectMake(self.bigScrollView.frame.size.width, 0, self.bigScrollView.frame.size.width, self.bigScrollView.frame.size.height);
    [self.bigScrollView addSubview:self.mineCorrectVC.view];
}
