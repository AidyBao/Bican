...
@interface MineCorrectTableViewCell ()
...
@property (nonatomic, strong) UIView *desclineView;
@property (nonatomic, strong) UIImageView *flowerImageView;
@property (nonatomic, strong) UILabel *flowerLabel;
@property (nonatomic, strong) UIView *flowerLineView;
@property (nonatomic, strong) UILabel *gradeLabel;
@property (nonatomic, strong) UIView *gradeLineView;

@end

@implementation MineCorrectTableViewCell
                            ...
                            ArticleFraction:(NSString *)articleFraction
{
    ...
    self.gradeLabel.text = [NSString stringWithFormat:@"我的评分：%@", articleFraction];
...
- (void)createSubViews
{
    ...
    //分割线
    self.desclineView = [[UIView alloc] init];
    self.desclineView.backgroundColor = ZTLineGrayColor;
    [self.contentView addSubview:self.desclineView];
    
    [self.desclineView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.headerImageView);
        make.right.equalTo(self.typeLabel);
    ...
    [self.flowerImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.headerImageView);
        make.top.equalTo(self.desclineView.mas_bottom).offset(GTH(20));
        make.size.mas_equalTo(CGSizeMake(GTH(35), GTH(35)));
    }];
    
    self.flowerLabel = [[UILabel alloc] init];
    self.flowerLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:GTW(24)];
    [self.contentView addSubview:self.titleLabel];
    ...
    [self.flowerLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.flowerImageView);
    }];
    
    //分割线
    self.flowerLineView = [[UIView alloc] init];
    self.flowerLineView.backgroundColor = ZTLineGrayColor;
    [self.contentView addSubview:self.flowerLineView];
    
    [self.flowerLineView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.headerImageView);
        make.right.equalTo(self.typeLabel);
        make.height.mas_equalTo(1);
        make.top.equalTo(self.flowerLabel.mas_bottom).offset(GTH(20));
    }];
    
    self.gradeLabel = [[UILabel alloc] init];
    self.gradeLabel.textColor = ZTTitleColor;
    self.gradeLabel.font = FONT(24);
    [self.contentView addSubview:self.gradeLabel];
    
    [self.gradeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.headerImageView);
        make.right.equalTo(self.typeLabel);
        make.top.equalTo(self.flowerLineView.mas_bottom).offset(GTH(20));
        make.height.mas_equalTo(GTH(40));
    }];
    
    //分割线
    self.gradeLineView = [[UIView alloc] init];
    self.gradeLineView.backgroundColor = ZTLineGrayColor;
    [self.contentView addSubview:self.gradeLineView];
    
    [self.gradeLineView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.headerImageView);
        make.right.equalTo(self.typeLabel);
        make.height.mas_equalTo(1);
//        make.top.equalTo(self.gradeLabel.mas_bottom).offset(GTH(20));
