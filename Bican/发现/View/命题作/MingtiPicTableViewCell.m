...
@interface MingtiPicTableViewCell ()
...
@implementation MingtiPicTableViewCell
...
- (void)createMingtiPicCellWith:(NSArray *)array
{
    ...
    CGFloat origin_x = GTW(28);
    CGFloat image_width = (ScreenWidth - origin_x * 4) / 3;
    
    for (int i = 0; i < array.count; i++) {
        
        UIImageView *imageView = [[UIImageView alloc] init];
        [imageView sd_setImageWithURL:[NSURL URLWithString:array[i]] placeholderImage:[UIImage imageNamed:@"zhanwei"]];
        ...
        [imageView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(self.contentView.mas_left).offset(image_width * i + origin_x * (i + 1));
        }];
...
- (void)imageButtonAction:(UIButton *)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(selectedImageWithCell:Index:)]) {
        [self.delegate selectedImageWithCell:self Index:sender.tag - 1001];
    }
