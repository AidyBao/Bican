//
//  CollectAndGoodViewController.m
//  Bican
//
...
#import "CollectAndGoodViewController.h"
#import "CollectAndGoodTableViewCell.h"
#import "MessageModel.h"
#import "ClassStudentViewController.h"

@interface CollectAndGoodViewController () <UITableViewDelegate, UITableViewDataSource, DZNEmptyDataSetSource, DZNEmptyDataSetDelegate>
...
@implementation CollectAndGoodViewController
...
{
    static NSString *defaultCellID = @"collectAndGoodTableViewCell";
    CollectAndGoodTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:defaultCellID];
    if (!cell) {
        cell = [[CollectAndGoodTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:defaultCellID];
    }
    ...
    [cell.headImageView sd_setImageWithURL:[NSURL URLWithString:model.avatar] placeholderImage:[UIImage imageNamed:@"sousuo_xiaoxi"]];
    
    cell.leftTopLabel.text = model.nickname;
    cell.leftBottomLabel.text = model.content;
    cell.rightTopLabel.text = model.pubTime;
    cell.rightBottomLabel.text = model.title;
    
    if ([model.wetherRead isEqualToString:@"0"]) {//未读
        [cell setCollectAndGoodCellIsShowWarning:@"YES"];
    } else {
        [cell setCollectAndGoodCellIsShowWarning:@"NO"];
    }
    return cell;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (leftBottomSize.height >= GTH(90)) {
        return GTH(20) * 2 + leftTopSize.height + GTH(90) + 1;

    } else {
        return GTH(20) * 2 + leftTopSize.height + leftBottomSize.height + 1;
