...
- (void)show
{
    ...
    if (self.animate) {
        CGPoint fromPoint = CGPointMake(ScreenWidth / 2, ScreenHeight + kBaseViewHeight / 2);
        CGPoint toPoint = CGPointMake(ScreenWidth / 2, ScreenHeight-kBaseViewHeight / 2);
...
- (IBAction)btnPhotoLibrary_Click:(id)sender
{
    if (![self judgeIsHavePhotoAblumAuthority]) {
        ...
        [photoBrowser setDoneBlock:^(NSArray<ZLSelectPhotoModel *> *selPhotoModels) {
            strongify(weakSelf);
...
- (void)cell_btn_Click:(UIButton *)btn
{
    ...
    if (!btn.selected) {
        ...
        if (![[ZLPhotoTool sharePhotoTool] judgeAssetisInLocalAblum:asset]) {
            [ZTToastUtils showToastIsAtTop:NO Message:@"该图片已存在" Time:3.0];
            return;
        }
...
- (void)changeBtnCameraTitle
{
    if (self.arraySelectPhotos.count > 0) {
        ...
        [self.btnCamera setTitleColor:RGBA(19, 153, 231, 1) forState:UIControlStateNormal];
...
{
    //无相册访问权限
    [ZTToastUtils showToastIsAtTop:NO Message:@"无相机访问权限" Time:3.0];
}
...
- (void)presentVC:(UIViewController *)vc
{
    ...
    [nav.navigationBar setBackgroundImage:[self imageWithColor:ZTOrangeColor] forBarMetrics:UIBarMetricsDefault];
    [nav.navigationBar setTintColor:ZTNavColor];
            ...
            [[ZLPhotoTool sharePhotoTool] saveImageToAblum:image completion:^(BOOL suc, PHAsset *asset) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (suc) {
                        ...
                        [ZTToastUtils showToastIsAtTop:NO Message:@"保存图片失败" Time:3.0];
                    }
                    [hud hide];
                    [strongSelf hide];
                });
